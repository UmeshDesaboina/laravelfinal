        ...
        <div class="flex flex-col lg:flex-row gap-8">
                        ...
                        <div class="space-y-3">
                            ...
                            @foreach($categories as $category)
                                <label class="flex items-center group cursor-pointer">
                                    ...
                                    <span class="text-sm font-medium group-hover:text-green-500 transition-colors {{ request('category') == $category->slug ? 'text-green-500' : '' }}">{{ $category->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="mb-10">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach(['S', 'M', 'L', 'XL', 'XXL'] as $size)
                                <button onclick="toggleSize('{{ $size }}')" class="size-btn w-10 h-10 rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center text-xs font-bold hover:border-green-500 hover:text-green-500 transition-all" data-size="{{ $size }}">{{ $size }}</button>
                            @endforeach
                        </div>
                    </div>
                </div>
            ...
            <div class="flex-1">
                ...
                <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 bg-white dark:bg-gray-800 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                    <p class="text-sm text-gray-500 font-medium ml-4">Showing <span id="product-count">{{ $products->total() }}</span> products</p>
