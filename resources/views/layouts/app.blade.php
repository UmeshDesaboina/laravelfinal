...
<body class="font-sans antialiased bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100">
    <div class="min-h-screen flex flex-col">
        <!-- Navigation -->
        <nav x-data="{ open: false, searchOpen: false, cartCount: 0 }" x-init="fetch('{{ route('cart.count') }}').then(r => r.json()).then(d => cartCount = d.count)" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 sticky top-0 z-50 shadow-sm">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    <!-- Logo -->
                    <div class="shrink-0 flex items-center">
                        <a href="{{ route('home') }}" class="text-2xl font-bold text-green-500 tracking-tight">
                    ...
                    <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                        <a href="{{ route('home') }}" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('home') ? 'border-green-500' : 'border-transparent' }} text-sm font-medium leading-5 transition duration-150 ease-in-out">Home</a>
                        <a href="{{ route('shop') }}" class="inline-flex items-center px-1 pt-1 border-b-2 {{ request()->routeIs('shop') ? 'border-green-500' : 'border-transparent' }} text-sm font-medium leading-5 transition duration-150 ease-in-out">Shop</a>
                    ...
                    <div class="flex items-center gap-6">
                        ...
                        <div class="relative hidden md:block" x-data="{ query: '', results: [] }">
                            <input type="text" x-model="query" @input.debounce.300ms="fetch(`{{ url('api/search-suggestions') }}?q=${query}`).then(r => r.json()).then(d => results = d)" placeholder="Search products..." class="w-64 rounded-full bg-gray-100 dark:bg-gray-700 border-none focus:ring-2 focus:ring-green-500 text-sm py-2 px-4">
                            <div x-show="query.length > 2 && results.length > 0" class="absolute top-full mt-2 w-full bg-white dark:bg-gray-800 rounded-xl shadow-xl border dark:border-gray-700 p-2 z-50" x-cloak>
                                <template x-for="item in results" :key="item.id">
                                    <a :href="`{{ url('products') }}/${item.slug}`" class="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors">
                        ...
                        @auth
                            <div class="flex items-center gap-4">
                                <a href="{{ route('wishlist.index') }}" class="relative text-gray-600 dark:text-gray-300 hover:text-green-500 transition-colors">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                                </a>
                                <a href="{{ route('cart.index') }}" class="relative text-gray-600 dark:text-gray-300 hover:text-green-500 transition-colors">
                                ...
                                <div class="relative" x-data="{ open: false }">
                                    <button @click="open = !open" class="flex items-center gap-2">
                                        <img src="https://ui-avatars.com/api/?name={{ auth()->user()->name }}" class="w-8 h-8 rounded-full border-2 border-green-500" alt="">
                                    </button>
                                    <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border dark:border-gray-700 py-2 z-50" x-cloak>
                                        @if(auth()->user()->isAdmin())
                                            <a href="{{ route('admin.dashboard') }}" class="block px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">Admin Panel</a>
                                        @endif
                                        <a href="{{ route('dashboard') }}" class="block px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">Dashboard</a>
                                        <a href="{{ route('profile.edit') }}" class="block px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">Profile</a>
                                        <a href="{{ route('orders.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-50 dark:hover:bg-gray-700">My Orders</a>
                        ...
                        @else
                            <div class="flex items-center gap-4">
                                <a href="{{ route('login') }}" class="text-sm font-medium hover:text-green-500 transition-colors">Login</a>
                                <a href="{{ route('register') }}" class="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-gray-800 dark:hover:bg-gray-100 transition-all shadow-lg shadow-gray-200 dark:shadow-none">Join Now</a>
                            </div>
                        @endauth
                    </div>
                </div>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
                    <div class="col-span-1 md:col-span-1">
                        <a href="{{ route('home') }}" class="text-2xl font-bold text-green-500 tracking-tight mb-6 block">
                            FIGHT<span class="text-gray-900 dark:text-white">WISDOM</span>
                        </a>
                    ...
                    <div>
                        ...
                        <ul class="space-y-4 text-sm text-gray-500 dark:text-gray-400">
                            <li><a href="{{ route('shop') }}" class="hover:text-green-500 transition-colors">All Products</a></li>
