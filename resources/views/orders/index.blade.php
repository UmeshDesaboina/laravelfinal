    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($orders->isEmpty())
            <div class="bg-white dark:bg-gray-800 rounded-[3rem] p-20 text-center shadow-sm">
                ...
                <a href="{{ route('shop') }}" class="inline-block bg-green-500 text-white px-8 py-4 rounded-full font-bold hover:bg-green-600 transition-all shadow-xl">Start Shopping</a>
            </div>
        @else
            <div class="space-y-6">
                @foreach($orders as $order)
                    <div class="bg-white dark:bg-gray-800 rounded-3xl p-8 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-center gap-8 group hover:shadow-xl transition-all">
                        <div class="flex items-center gap-8">
                            ...
                            <div>
                                <p class="text-xs font-black uppercase tracking-widest text-gray-400 mb-1">Placed on {{ $order->created_at->format('M d, Y') }}</p>
                                <h3 class="text-xl font-black text-gray-900 dark:text-white">₹{{ number_format($order->total, 2) }}</h3>
                                <div class="mt-2 flex items-center gap-2">
                                    <span class="w-2 h-2 rounded-full 
                                        @if($order->status === 'delivered') bg-green-500
                                        @elseif($order->status === 'cancelled') bg-red-500
                                        @else bg-blue-500 @endif"></span>
                                    <span class="text-xs font-bold uppercase tracking-wider text-gray-500">{{ ucfirst($order->status) }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                            <a href="{{ route('orders.show', $order) }}" class="px-8 py-4 rounded-2xl font-black border-2 border-gray-100 dark:border-gray-700 hover:border-green-500 hover:text-green-500 transition-all">View Details</a>
