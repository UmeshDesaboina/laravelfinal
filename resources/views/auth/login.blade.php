...
<div class="bg-white dark:bg-gray-800 rounded-[2rem] p-8 shadow-xl border border-gray-100 dark:border-gray-700">
    ...
    <p class="text-sm text-gray-600 dark:text-gray-400 text-center mb-8">
        Or <a href="{{ route('register') }}" class="font-medium text-green-500 hover:text-green-600">create a new account</a>
    </p>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm font-medium text-green-500 hover:text-green-600">
                    Forgot password?
                </a>
            @endif
        </div>
